...
const dataPath = path.join(sysRoot, '.wetcatlauncher')
...
/**
 * Check if the launcher should enable Discord RPC.
 * 
 * @param {boolean} def Optional. If true, the default value will be returned.
 * @returns {boolean} Whether or not the launcher should enable Discord RPC.
 */
exports.getDiscordRPC = function (def = false) {
    return !def ? config.settings.launcher.discordRPC : DEFAULT_CONFIG.settings.launcher.discordRPC
}

/**
 * Change the status of whether or not the launcher should enable Discord RPC.
 * 
 * @param {boolean} discordRPC Whether or not the launcher should enable Discord RPC.
 */
